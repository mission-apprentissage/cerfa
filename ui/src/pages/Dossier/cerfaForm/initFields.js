...
import { findDefinition } from "./formEngine/utils";
...
const createField = ({ name, data, schema }) => {
  ...
  return {
    ...
    // required: data ? !data.isNotRequiredForm : false,
    // validate: fieldSchema?.validate ?? undefined,
    ...fieldSchema,
    locked: data.locked,
    success: !isFalsyValue(data?.value),
    value,
    name,
  };
};

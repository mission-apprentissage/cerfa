...
readonly BACKUP_LOCAL_DIR="/opt/cerfa/backups/mongodb"
readonly BACKUP_FILE="${BACKUP_LOCAL_DIR}/mongodb-$(date +'%Y-%m-%d_%H%M%S').gpg"

function backup() {
  echo "Creating backup..."
  mkdir -p "${BACKUP_LOCAL_DIR}"
  docker exec cerfa_mongodb bash -c "mongodump --gzip --archive -u backup -p {{ vault[env_type].CERFA_MONGODB_BACKUP_PASSWORD }}" \

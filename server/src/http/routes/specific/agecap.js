    ...
    tryCatch(async ({ body, user }, res) => {
      ...
      const contratAgecap = {
        employeur: {
          ...
          telephone: cerfa.employeur.telephone.replace("+", ""),
        ...
        alternant: {
          ...
          telephone: cerfa.apprenti.telephone.replace("+", ""),
      ...
      try {
        ...
        await dossiers.updateStatutTransmission(dossierId, "SUCCESS", {
          statut_transmission_agecap_contrat_details: sendContratResponse.data.globalSuccess[0],
        });
      } catch (error) {
        console.log(error);
        await dossiers.updateStatutTransmission(dossierId, "ERROR_CONTRAT", {
          statut_transmission_agecap_contrat_details: error.reason.globalErrors[0],
        });
      ...
      if (contratAgecap.detailsContrat.infoTransmission.pj.length > 0 && sendContratResponse.status === 201) {
        ...
        await asyncForEach(dossier.documents, async (document) => {
          try {
            ...
            await dossiers.updateStatutTransmission(dossierId, "SUCCESS", {
              statut_transmission_agecap_pj_details: sendDocumentResponse.data.globalSuccess[0],
            });
            ...
            console.log(error);
            await dossiers.updateStatutTransmission(dossierId, "ERROR_PJ", {
              statut_transmission_agecap_pj_details: error.reason.globalErrors[0],
            });
            hasError = true;
            sendDocumentResponses.push({
              documentId: document.documentId,
